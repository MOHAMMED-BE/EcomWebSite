...
if(isset($_POST["prenom"])&&isset($_POST["nom"])&&isset($_POST["email"])&&isset($_POST["password"])&&isset($_POST["password2"])){
  ...
  if($_POST['password']!=$_POST['password2']){
    ...
    $req_verf=$conn->prepare("SELECT * from user where email=:email");
    $req_verf->bindParam(':email', $email);
    $req_verf->execute();
  
    $count=$req_verf->fetchColumn();
    if($count==0){
      $sql=$conn->prepare("insert into user(prenom,nom,email,password,role) values(:prenom,:nom,:email,:password,'Client')");
      $sql->execute(array(
        'prenom'=>$_POST["prenom"],
        'nom'=>$_POST["nom"],
        'email'=>$_POST["email"],
...
<head>
  ...
  <title>Pages / Register - OBM Admin Bootstrap Template</title>
            ...
            <div class="col-lg-4 col-md-6 d-flex flex-column align-items-center justify-content-center">
              ...
              <div class="d-flex justify-content-center py-4">
                <a href="index.php" class="logo d-flex align-items-center w-auto">
                  ...
                  <span class="d-none d-lg-block">OBM Admin</span>
                </a>
              </div><!-- End Logo -->
